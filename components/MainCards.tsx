...
const MainCards: React.FC = () => {
  return (
    <section className="py-24 px-4 md:px-0 bg-black/40">
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
        {cards.map((card, idx) => (
          <a 
            key={idx} 
            href={`#${card.title.toLowerCase()}`}
            className="group relative h-[700px] overflow-hidden reveal shadow-2xl"
          ...
          >
            ...
            ></div>
            <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent group-hover:bg-black/40 transition-all duration-700"></div>
            
            <div className="absolute inset-0 flex flex-col items-center justify-end p-16 text-white">
              <span className="text-[10px] font-black tracking-[0.5em] mb-4 text-brand-orange opacity-0 group-hover:opacity-100 transition-all translate-y-4 group-hover:translate-y-0">{card.label}</span>
              ...
              <div className="w-12 h-[1px] bg-white/30 group-hover:w-full group-hover:bg-brand-orange transition-all duration-700"></div>
            </div>
            
            {/* Border glow on hover */}
            <div className="absolute inset-0 border border-white/0 group-hover:border-white/10 transition-colors pointer-events-none"></div>
          </a>
        ))}
      </div>
    </section>
